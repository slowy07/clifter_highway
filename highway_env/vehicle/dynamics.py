...
class BicycleVehicle(Vehicle):
    ...
    def derivative(self) -> np.ndarray:
        ...
        delta_r = 0
        theta_vf = np.arctan2(
            self.lateral_speed + self.LENGTH_A * self.yaw_rate, self.speed
        )  # (2.27)
        theta_vr = np.arctan2(
            self.lateral_speed - self.LENGTH_B * self.yaw_rate, self.speed
        )  # (2.28)
        f_yf = 2 * self.FRICTION_FRONT * (delta_f - theta_vf)  # (2.25)
        f_yr = 2 * self.FRICTION_REAR * (delta_r - theta_vr)  # (2.26)
        if (
            abs(self.speed) < 1
        ):  # Low speed dynamics: damping of lateral speed and yaw rate
            f_yf = (
                -self.MASS * self.lateral_speed
                - self.INERTIA_Z / self.LENGTH_A * self.yaw_rate
            )
            f_yr = (
                -self.MASS * self.lateral_speed
                + self.INERTIA_Z / self.LENGTH_A * self.yaw_rate
        ...
        d_lateral_speed = (
            1 / self.MASS * (f_yf + f_yr) - self.yaw_rate * self.speed
        )  # (2.21)
        d_yaw_rate = (
